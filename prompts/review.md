...
- Migration and rollout risks: deploy order, compatibility, flags, backfills. If the PR introduces database migrations, verify it does not also introduce application code that depends on those migrations having already run. Migrations are not run automatically, but deploys are, so code can deploy before the migration has been applied. Flag migration-dependent code in the same PR as a blocker unless the code is safely compatible with both the pre- and post-migration schema. Newly added migrations should be the latest migration in `schema.sql`; adding migrations that are out of order (not at the end of the list) can result in subtle bugs because the order in our codebase doesn't match the order the migrations were applied in prod (the new migrations will still be applied in prod, but after the ones that look like they came later when looking at the code).
