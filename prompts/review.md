...
- Desktop build for testing: if the PR changes files under `packages/desktop` in the `foxglove/app` repo, a desktop build must be available for testing. Authors produce one by running the [`draft-release` workflow](https://github.com/foxglove/app/actions/workflows/draft-release.yml) against their branch and linking the resulting build in the Testing section of the PR description. A build link is required unless **every** changed file under `packages/desktop` matches an exempt pattern — stories (`**/*.stories.{ts,tsx}`) or tests (`**/*.test.{ts,tsx}`) — or the entire change is purely cosmetic copy/style with no behavioral effect; when in doubt, require the build. When a build is required, verify the link is present and reflects the current changes. Flag a missing or stale link (e.g. the description wasn't updated after later commits) as a blocker and ask the author to run the workflow and update the Testing section.
