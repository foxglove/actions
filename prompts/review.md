...
- Desktop build for testing (`foxglove/app` repo): some PRs need a desktop build linked in the Testing section of the description, produced by the [`draft-release` workflow](https://github.com/foxglove/app/actions/workflows/draft-release.yml) run against the branch. Require one when a PR (a) touches `packages/desktop`, or (b) clearly changes platform/OS-specific behavior — platform branching (`navigator.userAgent`, `process.platform`, `isMac`/`isWindows`/`isLinux`), Cmd-vs-Ctrl modifiers, native menus, file-system paths, clipboard, or a description that calls out OS-specific behavior. Lean toward under-reporting: skip when behavior doesn't actually differ by platform (e.g. logging `process.platform`), when it's unclear, or when every changed file is a story (`**/*.stories.{ts,tsx}`), test (`**/*.test.{ts,tsx}`), or purely cosmetic. When a build is required, treat a missing link as a blocker. For freshness, you can't see which commit a build came from, so compare timestamps via `gh api graphql`: if the latest commit's `committedDate` is after the PR body's `lastEditedAt` (or `createdAt`), the build may be stale — ask the author to confirm or rebuild rather than asserting it.
