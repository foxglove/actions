...
- Desktop build for testing: in the `foxglove/app` repo, a desktop build must be linked in the Testing section of the PR description when a PR (a) changes files under `packages/desktop`, or (b) changes behavior that differs by platform/OS — e.g. platform branching (`navigator.userAgent`, `process.platform`, `isMac`/`isWindows`/`isLinux`), Cmd-vs-Ctrl modifiers, native menus, file-system paths, or clipboard, including when the description calls out OS-specific behavior. Merely referencing the platform without differing behavior (e.g. logging `process.platform`) does not count. Builds come from the [`draft-release` workflow](https://github.com/foxglove/app/actions/workflows/draft-release.yml) run against the branch. Exempt a PR only when **every** changed file is a story (`**/*.stories.{ts,tsx}`) or test (`**/*.test.{ts,tsx}`), or the change is purely cosmetic copy/style; when in doubt, require the build. Otherwise, verify the link is present and current, and flag a missing or stale one (e.g. description not updated after later commits) as a blocker, asking the author to rebuild and update the Testing section.
