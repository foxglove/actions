...
2. For each of your prior threads (`CONTEXT.bot_login`) that is now fixed:
   - Reply on the thread: `gh api repos/<CONTEXT.repo>/pulls/<CONTEXT.pr_number>/comments/<COMMENT_ID>/replies -f body='<REPLY>'`, where `<COMMENT_ID>` is the thread's first comment.
