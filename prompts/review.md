...
- Migration and rollout risks: deploy order, compatibility, flags, backfills. If the PR introduces database migrations, verify it does not also introduce application code that depends on those migrations having already run. Migrations are not run automatically, but deploys are, so code can deploy before the migration has been applied. Flag migration-dependent code in the same PR as a blocker unless the code is safely compatible with both the pre- and post-migration schema. Newly added migrations should be the latest migration in `schema.sql`; if they are inserted in the past, warn about the risk that they may not be applied in the intended order.
