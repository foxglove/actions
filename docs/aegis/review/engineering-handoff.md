...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------- |
| Unattended authentication   | Qualify a separate company-owned issuer and exact-run private handoff with no runner superadmin access or employee credential dependency. Resolve session mode explicitly; verify I-01 and I-18–I-22. The normal app route returns success, not a raw token.                                                                   | E2                                       |
...
Use a dedicated transient GCS bucket with uniform access and public access prevention; disable versioning and soft delete, with no retention lock. This does not modify evidence or journal storage. A candidate exact-object read mechanism is a short-lived credential with a GCS Credential Access Boundary and a custom get-only role. Verify effective permissions, including inherited grants: the runner must have neither an unrestricted parent credential nor permission to acquire one. Object naming alone does not enforce isolation. Keep issuer and storage identities inaccessible through the harness environment, mounts and metadata network path.

An issuer-owned cleanup process deletes the expected object generation after login acknowledgment, cancellation or expiry, and handles missing acknowledgments/crashes independently of the runner. Configure bounded retries and a maximum cleanup delay, record actual deletion or explicit cleanup failure, and notify the responsible team through the approved operational channel without payloads. Lifecycle expiry is backup housekeeping, not a precise deletion deadline. A storage read is repeatable, and object deletion is not session revocation; qualify application single-use redemption, including concurrent attempts.
